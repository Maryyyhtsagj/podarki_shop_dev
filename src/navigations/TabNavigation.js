...
import {
  ...
  ShopDataName,
  WaitingName,
  MessagesName, SET_MESSAGES_COUNT,
} from '../constants';
import io from 'socket.io-client';
import {useEffect, useState} from 'react';
import { getFcmToken } from '../screens/authScreens/signinScreen/SignInScreen';
...
export default function TabNavigation() {
  ...
  useEffect(() => {
    let checkToken = async () => {
      let checkTok = await getFcmToken();
      console.log('checkTok', checkTok)
    };
  }, []);
  ...
  useEffect(() => {
    const socket1 = io.connect(
      `http://79.174.80.241:3001/api/count/orders/seller?store_id=${shop?._id}`,
    );
        ...
        tabBarIcon: ({focused, color, size}) => {
          ...
          if (!keyboardStatus) {
            ...
            if (route.name === ChatName) {
              ...
              chatIcon = homeState.messagesCount;
