...
import {BaseUrl, Colors, globalStyles, imageUrl, SET_CUSTOMER} from '../../../constants';
...
export const CreateShopScreen = ({navigation, route}) => {
  ...
  const [isCitySelection, setIsCitySelection] = useState(true);
  ...
  const onPressFunc = async () => {
    if (
        ...
        dateTime &&
        dateTime1 &&
        ((activeText === 'работает' && dateTime2 && dateTime3) ||
            activeText !== 'работает')
    ) {
      axiosFunc();
    } else if (!photo) {
    ...
    if (!routeShop) {
      ...
      try {
        ...
        const response = await axiosInstance.put(
            `/stores/my?store_id=${routeShop._id}`,
            formData,
            {
              headers: {
                'Content-Type': 'multipart/form-data',
  ...
  useEffect(() => {
    if (routeShop) {
      ...
      if (routeShop.weekdays) {
        setDateTime(routeShop.weekdays.from);
        setDateTime1(routeShop.weekdays.to);
      }

      if (routeShop.weekends) {
        if (routeShop.weekends.not_working) {
          setActiveText('не работает');
        } else {
          setActiveText('работает');
          setDateTime2(routeShop.weekends.from);
          setDateTime3(routeShop.weekends.to);
        }
      }
    }
  }, []);
  ...
  const searchDataYandex = st => {
    axios
        .get(
            `https://geocode-maps.yandex.ru/1.x?apikey=274503c6-0ff8-4554-a8a2-7a5d21d3bdfc&geocode=${
  ...
  const countryChangeFunc = (it, state) => {
    ...
    setLocation({
      ...
      lon: +stringArray[0],
      zoom: 8,
      name: it.GeoObject.metaDataProperty.GeocoderMetaData.Address
          ?.Components[2].name,
    });
    setSelectedCountry({
      lat: +stringArray[2],
  ...
  return (
      <ScrollView
          ...
          <View style={styles.contentContainer}>
            ...
            <View style={styles.dropCont}>
              ...
              <Text
                ...
                *Выставьте тариф доставки, если покупатель превысит зону
                бесплатной доставки в 5 км
              </Text>
            </View>
            <View>
              <Text
        ...
        <MapModal
            ...
            setFlag={setFlag}
            setFlag1={setFlag1}
            isCitySelection={isCitySelection}
        />
        <DatePicker
            modal
            open={open}
